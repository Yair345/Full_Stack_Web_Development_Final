...
const LoanApplicationForm = ({ onSubmit, loading, initialLoanType = 'personal' }) => {
    ...
    const getAvailableTerms = (loanType) => {
        return Object.keys(interestRateOptions[loanType] || {});
    };

    const [formData, setFormData] = useState({
        loan_type: initialLoanType,
        amount: '',
        interest_rate: getInterestRate(initialLoanType, '36'),
        term_months: '36',
        purpose: '',
    ...
    useEffect(() => {
        if (initialLoanType && initialLoanType !== formData.loan_type) {
            const availableTerms = getAvailableTerms(initialLoanType);
    ...
    const validateForm = () => {
        ...
        if (!formData.term_months) {
            ...
            const availableTerms = getAvailableTerms(formData.loan_type);
    ...
    const handleInputChange = (field, value) => {
        ...
        if (field === 'loan_type') {
            const availableTerms = getAvailableTerms(value);
            ...
            newFormData.interest_rate = getInterestRate(formData.loan_type, value);
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const defaultTerm = getAvailableTerms(initialLoanType)[0] || '36';
            setFormData({
                loan_type: initialLoanType,
                amount: '',
                interest_rate: getInterestRate(initialLoanType, defaultTerm),
                ...
                <div className="row g-3">
                    ...
                    <div className="col-md-6">
                        ...
                        <select
                            ...
                            required
                        >
                            {getAvailableTerms(formData.loan_type).map(term => (
